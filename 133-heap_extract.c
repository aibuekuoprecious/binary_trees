...
{
	heap_t *node_copy, *p, *grandparent, *left_child, *right_child;

	if ((node != NULL) && ((*node)->parent != NULL) && (root != NULL))
	{
		p = (*node)->parent, node_copy = *node, grandparent = (*node)->parent->parent;
		left_child = (*node)->left, right_child = (*node)->right;
		if ((*node)->parent->left != *node) /* swap from the right */
		{
			if ((grandparent != NULL) && (grandparent->left == (*node)->parent))
				grandparent->left = *node;
			if ((grandparent != NULL) && (grandparent->right == (*node)->parent))
				grandparent->right = *node;
			if ((*node)->parent->left != NULL)
				(*node)->parent->left->parent = node_copy;
			(*node)->parent = grandparent, (*node)->left = p->left, (*node)->right = p;
			p->parent = node_copy, p->left = left_child, p->right = right_child;
		}
		else /* swap from the left */
		{
			if ((grandparent != NULL) && (grandparent->left == p))
				grandparent->left = *node;
			if ((grandparent != NULL) && (grandparent->right == p))
				grandparent->right = *node;
			if ((*node)->parent->right != NULL)
				(*node)->parent->right->parent = *node;
			(*node)->parent = grandparent, (*node)->right = p->right, (*node)->left = p;
			p->parent = node_copy, p->left = left_child, p->right = right_child;
		}
		if (left_child != NULL)
			left_child->parent = p;
