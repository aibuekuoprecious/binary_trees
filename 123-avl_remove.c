...
/**
 * adjust_balance_1 - Adjusts the node_balance of an AVL tree by \
...
void adjust_balance_1(avl_t **root, avl_t *node)
{
	...
	while (cur != NULL)
	{
		node_balance = binary_tree_balance(cur);
		if (!((node_balance >= -1) && (node_balance <= 1)))
		{
			if ((node_balance == 2) && ((binary_tree_balance(cur->left) == 1)
				|| (binary_tree_balance(cur->left) == 0)))
			{
				new_root = binary_tree_rotate_right(cur);
			}
			else if ((node_balance == 2) && (binary_tree_balance(cur->left) == -1))
			{
				cur->left = binary_tree_rotate_left(cur->left);
				new_root = binary_tree_rotate_right(cur);
			}
			else if ((node_balance == -2) && ((binary_tree_balance(cur->right) == -1)
